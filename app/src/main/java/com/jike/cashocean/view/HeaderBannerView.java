...
import com.jike.cashocean.adapter.HeaderBannerAdapter;
...
public class HeaderBannerView extends AbsHeaderView<List<HomeListData.DataBean.DatasBean.BannerListBean>> {
...
//        bannerHeight = DensityUtil.getWindowWidth(context) * 9 / 16;
        bannerHeight = DensityUtil.dip2px(context, 94);
    }

    boolean addHead;

    @Override
    protected void getView(List<HomeListData.DataBean.DatasBean.BannerListBean> list, ListView listView) {
    ...
    private void dealWithTheView(List<HomeListData.DataBean.DatasBean.BannerListBean> list) {
        ivList.clear();
    ...
    // 创建要显示的ImageView
    private void createImageViews(List<HomeListData.DataBean.DatasBean.BannerListBean> list) {
//        for (int i = 0; i < list.size(); i++) {
//            GlideImageView imageView = new GlideImageView(mActivity);
//            AbsListView.LayoutParams params = new AbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
//            imageView.setLayoutParams(params);
//            imageView.loadImage(list.get(i).getBanner_image().toString(), R.color.font_black_6);
