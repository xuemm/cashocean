...
abstract class CameraController implements
    ...
    public void uncaughtException(final Thread thread, final Throwable throwable) {
        ...
        if (!(throwable instanceof CameraException)) {
            ...
            LOG.e("uncaughtException:", "Interrupting thread with state:", ss(), "due to " +
    ...
    // If closed, no-op. If opened, check supported and apply.
    abstract void setExposureCorrection(float EVvalue, float[] bounds, PointF[] points,
                                        boolean notify);
    ...
    // Throw if capturing. If in video session, recompute capture size, and, if needed, preview
    // size.
    ...
    protected final int computeSensorToViewOffset() {
        if (mFacing == Facing.FRONT) {
            ...
            // And it seemed to give the same results for various combinations, but not for all
            // (e.g. 0 - 270).
    ...
    protected final Size computePictureSize() {
        ...
        if (mSessionType == SessionType.PICTURE) {
            ...
            // And we want the picture size to be the biggest picture consistent with the video
            // aspect ratio.
            // -> Use the external picture selector, but enforce the ratio constraint.
            CamcorderProfile profile = getCamcorderProfile();
            AspectRatio targetRatio = AspectRatio.of(profile.videoFrameWidth,
                    profile.videoFrameHeight);
            if (flip) targetRatio = targetRatio.inverse();
            LOG.i("size:", "computeCaptureSize:", "videoQuality:", mVideoQuality, "targetRatio:",
                    targetRatio);
    ...
    protected final Size computePreviewSize(List<Size> previewSizes) {
        ...
        LOG.i("size:", "computePreviewSize:", "targetRatio:", targetRatio, "targetMinSize:",
                targetMinSize);
        SizeSelector matchRatio = SizeSelectors.aspectRatio(targetRatio, 0);
        ...
        SizeSelector matchAll = SizeSelectors.or(
                ...
                SizeSelectors.and(matchRatio, SizeSelectors.biggest()), // If couldn't match
                // both, match ratio and biggest.
