...
class Camera1 extends CameraController implements Camera.PreviewCallback, Camera.ErrorCallback {
    ...
    private void schedule(@Nullable final Task<Void> task, final boolean ensureAvailable,
    ...
    public void onSurfaceChanged() {
        ...
        schedule(null, true, new Runnable() {
            ...
            public void run() {
                ...
                Size newSize =
                        computePreviewSize(sizesFromList(mCamera.getParameters().getSupportedPreviewSizes()));
    ...
    private void applySizesAndStartPreview(String log) {
        ...
        params.setPreviewSize(mPreviewSize.getWidth(), mPreviewSize.getHeight()); // <- not
    ...
    void onStart() {
        ...
        if (collectCameraId()) {
            ...
            mCamera.setDisplayOrientation(computeSensorToViewOffset()); // <- not allowed during
            // preview
            if (shouldBindToSurface()) bindToSurface();
            LOG.i("onStart:", "Ended");
        }
    ...
    void setVideoQuality(VideoQuality videoQuality) {
        ...
        schedule(mVideoQualityTask, true, new Runnable() {
            ...
            public void run() {
                if (mIsCapturingVideo) {
                    // TODO: actually any call to getParameters() could fail while recording a
                    //  video.
                    // See. https://stackoverflow.com/questions/14941625/
                    mVideoQuality = old;
                    throw new IllegalStateException("Can't change video quality while recording a" +
                            " video.");
                }
                ...
                mCamera.takePicture(
                        ...
                        new Camera.PictureCallback() {
                            ...
                            public void onPictureTaken(byte[] data, final Camera camera) {
                                ...
                                camera.startPreview(); // This is needed, read somewhere in the
                                // docs.
                            }
                        }
                );
            ...
            public void run() {
                ...
                mCamera.setOneShotPreviewCallback(new Camera.PreviewCallback() {
                    ...
                    public void onPreviewFrame(final byte[] data, Camera camera) {
                        ...
                        // EXIF tags automatically set by camera. So either we add EXIF, or we
                        // rotate.
                        // Adding EXIF to a byte array, unfortunately, is hard.
                        final int sensorToOutput = computeSensorToOutputOffset();
                        final int sensorToView = computeSensorToViewOffset();
                        final boolean outputMatchesView =
                                (sensorToOutput + sensorToView + 180) % 180 == 0;
                        final boolean outputFlip = mFacing == Facing.FRONT;
                        ...
                        WorkerHandler.run(new Runnable() {
                            ...
                            public void run() {
                                ...
                                byte[] rotatedData = RotationHelper.rotate(data, preWidth,
                                        preHeight, sensorToOutput);
                                LOG.v("captureSnapshot: rotated.");
                                YuvImage yuv = new YuvImage(rotatedData, format, postWidth,
                                        postHeight, null);
                                mCameraCallbacks.processSnapshot(yuv, outputMatchesView,
                                        outputFlip);
                                mIsCapturingImage = false;
                            }
                        });

                        // It seems that the buffers are already cleared here, so we need to
                        // allocate again.
                        mCamera.setPreviewCallbackWithBuffer(null); // Release anything left
                        mCamera.setPreviewCallbackWithBuffer(Camera1.this); // Add ourselves
                        mFrameManager.allocate(ImageFormat.getBitsPerPixel(mPreviewFormat),
                                mPreviewSize);
                    }
                });
            }
        ...
        schedule(mStartVideoTask, true, new Runnable() {
            ...
            public void run() {
                ...
                if (mSessionType == SessionType.VIDEO) {
                    ...
                    throw new IllegalStateException("Can't record video while session type is " +
                            "picture");
                }
            }
        });
    ...
    void startAutoFocus(@Nullable final Gesture gesture, final PointF point) {
        ...
        schedule(null, true, new Runnable() {
            ...
            public void run() {
                ...
                // At this point we are sure that camera supports auto focus... right? Look at
    ...
    private static Rect computeMeteringArea(double centerX, double centerY, double size) {
        ...
        LOG.i("focus:", "computeMeteringArea:", "top:", top, "left:", left, "bottom:", bottom,
                "right:", right);
