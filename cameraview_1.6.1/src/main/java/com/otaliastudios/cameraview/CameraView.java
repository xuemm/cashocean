...
public class CameraView extends FrameLayout implements LifecycleObserver {
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.CameraView, 0
                , 0);

        // Self managed
        int jpegQuality = a.getInteger(R.styleable.CameraView_cameraJpegQuality,
                DEFAULT_JPEG_QUALITY);
        boolean cropOutput = a.getBoolean(R.styleable.CameraView_cameraCropOutput,
                DEFAULT_CROP_OUTPUT);
        boolean playSounds = a.getBoolean(R.styleable.CameraView_cameraPlaySounds,
                DEFAULT_PLAY_SOUNDS);

        // Camera controller params
        Facing facing = Facing.fromValue(a.getInteger(R.styleable.CameraView_cameraFacing,
                Facing.DEFAULT.value()));
        Flash flash = Flash.fromValue(a.getInteger(R.styleable.CameraView_cameraFlash,
                Flash.DEFAULT.value()));
        Grid grid = Grid.fromValue(a.getInteger(R.styleable.CameraView_cameraGrid,
                Grid.DEFAULT.value()));
        WhiteBalance whiteBalance =
                WhiteBalance.fromValue(a.getInteger(R.styleable.CameraView_cameraWhiteBalance,
                        WhiteBalance.DEFAULT.value()));
        VideoQuality videoQuality =
                VideoQuality.fromValue(a.getInteger(R.styleable.CameraView_cameraVideoQuality,
                        VideoQuality.DEFAULT.value()));
        SessionType sessionType =
                SessionType.fromValue(a.getInteger(R.styleable.CameraView_cameraSessionType,
                        SessionType.DEFAULT.value()));
        Hdr hdr = Hdr.fromValue(a.getInteger(R.styleable.CameraView_cameraHdr,
                Hdr.DEFAULT.value()));
        Audio audio = Audio.fromValue(a.getInteger(R.styleable.CameraView_cameraAudio,
                Audio.DEFAULT.value()));
        VideoCodec codec =
                VideoCodec.fromValue(a.getInteger(R.styleable.CameraView_cameraVideoCodec,
        ...
        // Gestures
        GestureAction tapGesture =
                GestureAction.fromValue(a.getInteger(R.styleable.CameraView_cameraGestureTap,
                        GestureAction.DEFAULT_TAP.value()));
        GestureAction longTapGesture =
                GestureAction.fromValue(a.getInteger(R.styleable.CameraView_cameraGestureLongTap,
                        GestureAction.DEFAULT_LONG_TAP.value()));
        GestureAction pinchGesture =
                GestureAction.fromValue(a.getInteger(R.styleable.CameraView_cameraGesturePinch,
                        GestureAction.DEFAULT_PINCH.value()));
        GestureAction scrollHorizontalGesture =
                GestureAction.fromValue(a.getInteger(R.styleable.CameraView_cameraGestureScrollHorizontal, GestureAction.DEFAULT_SCROLL_HORIZONTAL.value()));
        GestureAction scrollVerticalGesture =
                GestureAction.fromValue(a.getInteger(R.styleable.CameraView_cameraGestureScrollVertical, GestureAction.DEFAULT_SCROLL_VERTICAL.value()));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // should do,
        // other than respect it. The preview will eventually be cropped at the sides (by
        // PreviewImpl scaling)
        // except the case in which these fixed dimensions manage to fit exactly the preview
        // aspect ratio.
        ...
        // It's sure now that at least one dimension can be determined (either because EXACTLY or
        // AT_MOST).
        ...
        // but it is not guaranteed to succeed. It depends on the AT_MOST value of the other
        // dimensions.
        if (widthMode == EXACTLY || heightMode == EXACTLY) {
    ...
    protected boolean checkPermissions(SessionType sessionType, Audio audio) {
        ...
        needsCamera =
                needsCamera && c.checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED;
        needsAudio =
                needsAudio && c.checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED;
    ...
    /**
     * Sets picture capture size. The {@link SizeSelector} will be invoked with the list of
     * available
    ...
    // TODO: pauseCapturingVideo and resumeCapturingVideo. There is mediarecorder.pause(), but
    //  API 24...
    ...
    private class Callbacks implements CameraCallbacks {
        ...
        /**
         * What would be great here is to ensure the EXIF tag in the jpeg is consistent with what
         * we expect,
         ...
         * - You don't know what byte[] is, see
         * {@link android.hardware.Camera.Parameters#setRotation(int)}.
         * Sometimes our rotation is encoded in the byte array, sometimes a rotated byte[] is
         * returned.
         * Depends on the hardware.
         * <p>
         * So for now we ignore flipping.
         *
         * @param consistentWithView is the final image (decoded respecting EXIF data) consistent
         *                           with
         *                           the view width and height? Or should we flip dimensions to
         *                           have a
         *                           consistent measure?
         * @param flipHorizontally   whether this picture should be flipped horizontally after
         *                           decoding,
         *                           because it was taken with the front camera.
         */
        @Override
        public void processImage(final byte[] jpeg, final boolean consistentWithView,
            ...
            mWorkerHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mCropOutput && mCameraPreview.isCropping()) {
                        // If consistent, dimensions of the jpeg Bitmap and dimensions of
                        // getWidth(), getHeight()
                        ...
                        mLogger.i("processImage", "viewWidth?", getWidth(), "viewHeight?",
                                getHeight());
        ...
        @Override
        public void processSnapshot(final YuvImage yuv, final boolean consistentWithView,
            ...
            mWorkerHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mCropOutput && mCameraPreview.isCropping()) {
                        ...
                        mLogger.i("processSnapshot", "viewWidth?", getWidth(), "viewHeight?",
                                getHeight());
                        jpeg = CropHelper.cropToJpeg(yuv, targetRatio, mJpegQuality);
                    } else {
                        ByteArrayOutputStream out = new ByteArrayOutputStream();
                        yuv.compressToJpeg(new Rect(0, 0, yuv.getWidth(), yuv.getHeight()),
                                mJpegQuality, out);
        ...
        public void dispatchFrame(final Frame frame) {
            if (mFrameProcessors.isEmpty()) {
                ...
                mLogger.v("dispatchFrame:", frame.getTime(), "processors:",
                        mFrameProcessors.size());
